...
void ToolUtil::writeIniFileByInfo(QMap<QString,QVariant> map, QString filePath)
{
	QFile file(filePath);  
	...
	QSettings iniWrite(filePath, QSettings::IniFormat);   
	QMap<QString, QVariant>::iterator iter = map.begin();
	while (iter != map.end())
	{
		iniWrite.setValue(iter.key(), iter.value());
		iter++;
	}
...
void ToolUtil::writeFileByInfo(QList<QString> list, QString filePath)
{
	QFile f(filePath);  
	if(!f.open(QIODevice::WriteOnly | QIODevice::Text))  
	{  
...
void ToolUtil::setComboxCustomData(QComboBox *combox, QString displayValue, AType atype, ObId obId)
{
	...
	if (atype==GlobalInfo::AT_AlarmStateContainerLink)
	{
		...
		map = GlobalInfo::allUnitsLinkCombox;
	}else{
...
QString ToolUtil::joinTemplate(QList<AType> atypeList,ObId obId,OType otype,int& index,QMap<ObId,int> mvMap,QMap<ObId,int> dpsMap,ChoiceData deviceProperty,QString description)
{
	...
	QString data = "";
	data.append(QString::number(++index)).append(";").append(description).append(";").append(deviceProperty_s).append(";").append("");
...
StringData ToolUtil::QStringTostdstring(QString strData)
{
	return strData.toUtf8().data();
}
QString ToolUtil::StringDataToQString(StringData str)
{
	OMString  strStd = str;
	return QString::fromUtf8(strStd.c_str());
}